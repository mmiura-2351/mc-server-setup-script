...
pub async fn download_vanilla_server(version: &str) -> Result<String, String> {
    ...
    let download_url = version_details["downloads"]["server"]["url"]
        .as_str()
...
pub async fn download_forge_installer(version: &str) -> Result<String, String> {
    ...
    let response = client.get(promotions_url)
        .header(USER_AGENT, "MyMinecraftServerDownloader/1.0")
        .send()
        .await
    ...
    let forge_version = promotions["promos"]
        ...
        .ok_or("Specified Forge version not found")?.1
        .as_str()
        .ok_or("Invalid promotions format")?;
    
    let forge_url = format!("https://maven.minecraftforge.net/net/minecraftforge/forge/{version}-{forge_version}/forge-{version}-{forge_version}-installer.jar", version=version, forge_version=forge_version);
